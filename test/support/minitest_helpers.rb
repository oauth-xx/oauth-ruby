module OAuth
  module MinitestHelpers
    ...
    def stub_test_ie
      ...
      stub_request(:get, %r{http://term\.ie/oauth/example/echo_api\.php\?.+}).to_return(->(request) { { body: request.uri.query } })
      stub_request(:post, "http://term.ie/oauth/example/echo_api.php").to_return(->(request) { { body: request.body } })
    end
  end
end
