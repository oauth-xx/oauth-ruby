...
begin
  ...
  class EmHttpRequestProxyTest < Minitest::Test
    ...
    def test_request_proxy_ignores_post_body_with_invalid_method
      proxy = create_request_proxy head: { "Content-Type" => "application/x-www-form-urlencoded" },
                                   method: "DELETE"
      assert_empty(proxy.parameters)
    ...
    def test_request_proxy_works_with_argument_params
      assert_equal({ "a" => ["1"] },
                   create_request_proxy(proxy_options: { parameters: { "a" => "1" } }).parameters)
    end

    def test_request_proxy_works_with_mixed_params
      proxy = create_request_proxy(proxy_options: { parameters: { "a" => "1" } }, query: { "c" => "1" },
                                   uri: "http://example.com/test?b=1")
