...
class NetHTTPClientTest < Minitest::Test
  ...
  def test_that_using_auth_headers_on_get_requests_works_with_plaintext
    ...
    assert_equal "GET", request.method
    assert_equal "/test?key=value", request.path
    assert_matching_headers "oauth_nonce=\"225579211881198842005988698334675835446\", oauth_signature_method=\"PLAINTEXT\", oauth_token=\"token_411a7f\", oauth_timestamp=\"1199645624\", oauth_consumer_key=\"consumer_key_86cad9\", oauth_signature=\"5888bf0345e5d237%263196ffd991c8ebdb\", oauth_version=\"1.0\"",
  ...
  def test_that_body_hash_is_obmitted_when_no_algorithm_is_defined
    ...
    request.oauth!(@http, @consumer, @token,
                   { nonce: @nonce, timestamp: @timestamp, signature_method: "plaintext" })
  ...
  def test_that_using_get_params_works
    ...
    assert_equal "key=value&oauth_consumer_key=consumer_key_86cad9&oauth_nonce=225579211881198842005988698334675835446&oauth_signature=1oO2izFav1GP4kEH2EskwXkCRFg%3D&oauth_signature_method=HMAC-SHA1&oauth_timestamp=1199645624&oauth_token=token_411a7f&oauth_version=1.0",
  ...
  def test_that_using_get_params_works_with_plaintext
    ...
    assert_equal "/test", uri.path
    assert_nil uri.fragment
    assert_equal "key=value&oauth_consumer_key=consumer_key_86cad9&oauth_nonce=225579211881198842005988698334675835446&oauth_signature=5888bf0345e5d237%263196ffd991c8ebdb&oauth_signature_method=PLAINTEXT&oauth_timestamp=1199645624&oauth_token=token_411a7f&oauth_version=1.0",
                 uri.query.split("&").sort.join("&")
    assert_nil request["authorization"]
  end
  ...
  def test_that_using_post_params_works
    ...
    assert_equal "/test", request.path
    assert_match(
      /key=value&oauth_consumer_key=consumer_key_86cad9&oauth_nonce=225579211881198842005988698334675835446&oauth_signature=26g7wHTtNO6ZWJaLltcueppHYiI%3[Dd]&oauth_signature_method=HMAC-SHA1&oauth_timestamp=1199645624&oauth_token=token_411a7f&oauth_version=1.0/, request.body.split("&").sort.join("&")
    )
    assert_nil request["authorization"]
  end
  ...
  def test_that_using_post_params_works_with_plaintext
    ...
    request.set_form_data(@request_parameters)
    request.oauth!(@http, @consumer, @token,
                   { scheme: "body", nonce: @nonce, timestamp: @timestamp, signature_method: "PLAINTEXT" })

    assert_equal "POST", request.method
    assert_equal "/test", request.path
    assert_equal "key=value&oauth_consumer_key=consumer_key_86cad9&oauth_nonce=225579211881198842005988698334675835446&oauth_signature=5888bf0345e5d237%263196ffd991c8ebdb&oauth_signature_method=PLAINTEXT&oauth_timestamp=1199645624&oauth_token=token_411a7f&oauth_version=1.0",
                 request.body.split("&").sort.join("&")
    assert_nil request["authorization"]
  end
  ...
  def test_that_using_post_body_works
    ...
    assert_equal "/test", request.path
    assert_match(
      /OAuth oauth_consumer_key="consumer_key_86cad9", oauth_nonce="225579211881198842005988698334675835446", oauth_signature="%2[fF]DMMBOJzQ6JmEaXlAXDLGtD1z2I%3D", oauth_signature_method="HMAC-SHA1", oauth_timestamp="1199645624", oauth_token="token_411a7f", oauth_version="1.0"/, request["authorization"].split("&").sort.join("&")
    )
    # assert_nil request['authorization']
  end
  ...
  def test_that_using_post_with_uri_params_works
    ...
    assert_equal "/test", uri.path
    assert_nil uri.fragment
    assert_equal "key=value&oauth_consumer_key=consumer_key_86cad9&oauth_nonce=225579211881198842005988698334675835446&oauth_signature=26g7wHTtNO6ZWJaLltcueppHYiI%3D&oauth_signature_method=HMAC-SHA1&oauth_timestamp=1199645624&oauth_token=token_411a7f&oauth_version=1.0",
                 uri.query.split("&").sort.join("&")
  ...
  def test_that_using_post_with_uri_and_form_params_works
    ...
    assert_equal "/test", uri.path
    assert_nil uri.fragment
    assert_equal "key=value&oauth_consumer_key=consumer_key_86cad9&oauth_nonce=225579211881198842005988698334675835446&oauth_signature=4kSU8Zd1blWo3W6qJH7eaRTMkg0%3D&oauth_signature_method=HMAC-SHA1&oauth_timestamp=1199645624&oauth_token=token_411a7f&oauth_version=1.0",
                 uri.query.split("&").sort.join("&")
  ...
  def test_example_from_specs
    ...
    #    request = Net::HTTP::Get.new(request_uri.path + "?" + request_uri.query)
    request.oauth!(http, consumer, token,
                   { nonce: nonce, timestamp: timestamp, realm: "http://photos.example.net/" })
  ...
  def test_step_by_step_token_request
    ...
    signature_base_string = request.signature_base_string(http, consumer, nil,
                                                          { scheme: :query_string, nonce: nonce, timestamp: timestamp })
    assert_equal "GET&http%3A%2F%2Fterm.ie%2Foauth%2Fexample%2Frequest_token.php&oauth_consumer_key%3Dkey%26oauth_nonce%3D#{nonce}%26oauth_signature_method%3DHMAC-SHA1%26oauth_timestamp%3D#{timestamp}%26oauth_version%3D1.0",
                 signature_base_string
  ...
  def test_that_put_bodies_signed
    ...
    signature_base_string = request.signature_base_string(@http, @consumer, nil,
                                                          { nonce: @nonce, timestamp: @timestamp })
    assert_equal "PUT&http%3A%2F%2Fexample.com%2Ftest&oauth_body_hash%3DDvAa1AWdFoH9K%252B%252F2AHm3f6wH27k%253D%26oauth_consumer_key%3Dconsumer_key_86cad9%26oauth_nonce%3D225579211881198842005988698334675835446%26oauth_signature_method%3DHMAC-SHA1%26oauth_timestamp%3D1199645624%26oauth_version%3D1.0",
                 signature_base_string
  end
  ...
  def test_that_put_bodies_not_signed_even_if_form_urlencoded
    ...
    signature_base_string = request.signature_base_string(@http, @consumer, nil,
                                                          { nonce: @nonce, timestamp: @timestamp })
    assert_equal "PUT&http%3A%2F%2Fexample.com%2Ftest&key2%3Dvalue2%26oauth_consumer_key%3Dconsumer_key_86cad9%26oauth_nonce%3D225579211881198842005988698334675835446%26oauth_signature_method%3DHMAC-SHA1%26oauth_timestamp%3D1199645624%26oauth_version%3D1.0",
                 signature_base_string
  end
  ...
  def test_that_post_bodies_signed_if_form_urlencoded
    ...
    signature_base_string = request.signature_base_string(@http, @consumer, nil,
                                                          { nonce: @nonce, timestamp: @timestamp })
    assert_equal "POST&http%3A%2F%2Fexample.com%2Ftest&key2%3Dvalue2%26oauth_consumer_key%3Dconsumer_key_86cad9%26oauth_nonce%3D225579211881198842005988698334675835446%26oauth_signature_method%3DHMAC-SHA1%26oauth_timestamp%3D1199645624%26oauth_version%3D1.0",
                 signature_base_string
  end
  ...
  def test_that_post_bodies_signed_if_other_content_type
    ...
    signature_base_string = request.signature_base_string(@http, @consumer, nil,
                                                          { nonce: @nonce, timestamp: @timestamp })
    assert_equal "POST&http%3A%2F%2Fexample.com%2Ftest&oauth_body_hash%3DDvAa1AWdFoH9K%252B%252F2AHm3f6wH27k%253D%26oauth_consumer_key%3Dconsumer_key_86cad9%26oauth_nonce%3D225579211881198842005988698334675835446%26oauth_signature_method%3DHMAC-SHA1%26oauth_timestamp%3D1199645624%26oauth_version%3D1.0",
                 signature_base_string
  end
  ...
  def assert_no_match(exp, act, _deprecated = nil)
    refute_match(exp, act)
  end
